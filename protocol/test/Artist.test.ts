...
async function testArtistContract(deployContract: Function, name: string) {
  ...
  describe('buyEdition', () => {
    ...
    it(`reverts if permissioned is sold out and open auction hasn't started`, async () => {
      ...
      const tx1 = await artist.connect(buyer).buyEdition(EDITION_ID, signature, ticketNumber, {
        value: price,
      });
    ...
    it(`reverts if signature is invalid during permissioned sale`, async () => {
      await setUpContract({
        permissionedQuantity: BigNumber.from(1),
    ...
    it(`reverts if signature is signed by wrong address during permissioned sale`, async () => {
      const quantity = 1;
      await setUpContract({
      ...
      const tx = artist.connect(buyer).buyEdition(EDITION_ID, signature, ticketNumber, {
        value: price,
      });
